...
#print(" ; ".join(listCmd))
#listOutput = subprocess.getoutput(" ; ".join(listCmd))
with open('result.txt','w') as f_out:
    subprocess.call(" ; ".join(listCmd), shell=True, stdout=f_out)
...
tmp = hSigGen.Clone()
for i in range(1, nNBinGen+1):
    tmp.SetBinContent(i,listResVal[i-1])
    tmp.SetBinError(i,listResErr[i-1])
...
c.Clear()
hData.Draw("colz")
c.Print("tmp2.pdf","pdf")
c.Clear()
tmptmptmp = hData.ProjectionX()
tmptmptmp.Draw("hist")
c.Print("tmp3.pdf", "pdf")
tmptmp = h3RespMat.Project3D("xy")
tmptmp.Draw("colz")
c.Print("tmp4.pdf", "pdf")
